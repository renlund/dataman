##' fix censored dates
##'
##' fix censored dates of the type '20110000' or '20110200' by selecting the
...
##'     error
##' @export
fix_cens_date <- function(x, sep = NULL, low.bound = NULL, ok.years = NULL){
    if(L <- !is.null(low.bound)){
        if(length(x) != length(low.bound)) stop("want same length of bound as x")
    }
    n <- length(x)
    if(n==0) return(NULL)
    r <- as.Date(rep(NA, n))
...
# - #' helper for fix_cens_date
fix1censdate <- function(x, sep = NULL, low.bound = NULL, ok.years = NULL){
    ...
    if(!is.null(sep)){
        x <- paste0(unlist(strsplit(x, split = sep, fixed = TRUE)), collapse = "")
    }
    if(nchar(x) != 8){
